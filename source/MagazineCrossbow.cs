...
public class MagazineCrossbowClient : RangeWeaponClient
{
    ...
    protected virtual bool OpenLid(ItemSlot slot, EntityPlayer player, ref int state, ActionEventData eventData, bool mainHand, AttackDirection direction)
    {
        ...
        Api.World.RegisterCallback(_ => OpenLidCallback(), GetActionDelayMs(player, openLidAnimation, animationSpeed, 600));

        return true;
    }
    protected virtual bool OpenLidCallback()
    {
        if ((MagazineCrossbowState)(PlayerBehavior?.GetState(mainHand: true) ?? 0) != MagazineCrossbowState.OpenLid) return true;
    ...
    {
        if ((MagazineCrossbowState)(PlayerBehavior?.GetState(mainHand: true) ?? 0) != MagazineCrossbowState.Load) return true;
    ...
    {
        if ((MagazineCrossbowState)(PlayerBehavior?.GetState(mainHand) ?? 0) != MagazineCrossbowState.CloseLid) return true;
    ...
    {
        if ((MagazineCrossbowState)(PlayerBehavior?.GetState(mainHand) ?? 0) != MagazineCrossbowState.Shoot) return true;
