...
public class CrossbowClient : RangeWeaponClient
{
    ...
    public override void OnSelected(ItemSlot slot, EntityPlayer player, bool mainHand, ref int state)
    {
        ...
        if (drawn)
        {
            ...
            TpAnimationBehavior?.Play(mainHand, GetThirdPersonItemAnimation(mainHand ? player.RightHandItemSlot : player.LeftHandItemSlot, Stats.LoadedAnimation), category: "string", weight: 0.001f);
        }
        else
        {
    ...
    public override void OnRegistered(ActionsManagerPlayerBehavior behavior, ICoreClientAPI api)
    {
        ...
        AnimationBehavior = behavior.entity.GetBehavior<CombatOverhaul.Animations.FirstPersonAnimationsBehavior>();
        TpAnimationBehavior = behavior.entity.GetBehavior<CombatOverhaul.Animations.ThirdPersonAnimationsBehavior>();
        AimingAnimationController = new(AimingSystem, AnimationBehavior, AimingStats);
    }

    private static string GetThirdPersonItemAnimation(ItemSlot slot, string animation)
    {
        if (string.IsNullOrEmpty(animation) || animation.EndsWith("-tp", StringComparison.Ordinal)) return animation;

        string itemCode = slot.Itemstack?.Collectible?.Code?.Path ?? string.Empty;

        // These crossbow item shapes use separate third-person item animation tracks.
        // Playing the first-person item animation on the TP renderer leaves the visible
        // crossbow static, while attachments such as the bolt still animate on the parent.
        if (itemCode.Contains("-goatsfoot-", StringComparison.Ordinal)
            || itemCode.Contains("-stirrup-", StringComparison.Ordinal)
            || itemCode.Contains("-windlass-", StringComparison.Ordinal)
            || itemCode.Contains("-repeating-", StringComparison.Ordinal))
        {
            return animation + "-tp";
        }
    ...
    protected new CombatOverhaul.Animations.FirstPersonAnimationsBehavior? AnimationBehavior;
    protected new CombatOverhaul.Animations.ThirdPersonAnimationsBehavior? TpAnimationBehavior;
    ...
    protected virtual bool Draw(ItemSlot slot, EntityPlayer player, ref int state, ActionEventData eventData, bool mainHand, AttackDirection direction)
    {
        ...
        string drawAnimation = Stats.DrawAnimation;

        AnimationBehavior?.Play(mainHand, drawAnimation, animationSpeed: animationSpeed);
        TpAnimationBehavior?.Play(mainHand, GetThirdPersonItemAnimation(slot, drawAnimation), animationSpeed: animationSpeed);
        ...
        Api.World.RegisterCallback(_ => DrawAnimationCallback(slot, mainHand, player), GetActionDelayMs(player, drawAnimation, animationSpeed, 1500));
    ...
    protected virtual bool Load(ItemSlot slot, EntityPlayer player, ref int state, ActionEventData eventData, bool mainHand, AttackDirection direction)
    {
        ...
        ItemStackRangedStats stackStats = ItemStackRangedStats.FromItemStack(slot.Itemstack);
        float animationSpeed = GetAnimationSpeed(player, Stats.ProficiencyStat) * stackStats.ReloadSpeed * Stats.ReloadAnimationSpeed;
        string loadAnimation = Stats.LoadAnimation;
        ...
        AnimationBehavior?.Play(mainHand, loadAnimation, animationSpeed: animationSpeed);
        TpAnimationBehavior?.Play(mainHand, GetThirdPersonItemAnimation(slot, loadAnimation), animationSpeed: animationSpeed);
        ...
        Api.World.RegisterCallback(_ => LoadAnimationCallback(slot, mainHand, player), GetActionDelayMs(player, loadAnimation, animationSpeed, 900));
    ...
    protected virtual bool Aim(ItemSlot slot, EntityPlayer player, ref int state, ActionEventData eventData, bool mainHand, AttackDirection direction)
    {
        ...
        TpAnimationBehavior?.Play(mainHand, GetThirdPersonItemAnimation(slot, Stats.AimAnimation));
    ...
    protected virtual bool Shoot(ItemSlot slot, EntityPlayer player, ref int state, ActionEventData eventData, bool mainHand, AttackDirection direction)
    {
        ...
        string releaseAnimation = Stats.ReleaseAnimation;
        ...
        TpAnimationBehavior?.Play(
            mainHand,
            GetThirdPersonItemAnimation(slot, releaseAnimation),
            weight: 1000);
        ...
        Api.World.RegisterCallback(_ => ReleaseAnimationCallbackHandler("shoot", slot, mainHand, player), GetActionDelayMs(player, releaseAnimation, 1, 100, "shoot"));
        Api.World.RegisterCallback(_ => ReleaseAnimationCallback(slot, mainHand, player), GetActionDelayMs(player, releaseAnimation, 1, 500));
    ...
    {
        if ((CrossbowState)(PlayerBehavior?.GetState(mainHand) ?? 0) != CrossbowState.Shooting) return;
    ...
    protected virtual ItemSlot? GetBoltSlot(EntityPlayer player)
    {
        ...
        if (boltSlot == null)
        {
            player.WalkInventory(slot =>
            {
                ...
                if (slot.Itemstack.Item.GetCollectibleBehavior<ProjectileBehavior>(true) != null && WildcardUtil.Match(Stats.BoltWildcard, slot.Itemstack.Item.Code.ToString()))
                {
                    boltSlot = slot;
                    return false;
                }
...
public class CrossbowServer : RangeWeaponServer
{
    ...
    {
        if (ammoSlot?.Itemstack?.Item != null && ammoSlot.Itemstack.Item.GetCollectibleBehavior<ProjectileBehavior>(true) != null && WildcardUtil.Match(_stats.BoltWildcard, ammoSlot.Itemstack.Item.Code.ToString()))
        {
...
public sealed class CrossbowItem : Item, IHasWeaponLogic, IHasRangedWeaponLogic, IHasIdleAnimations
{
    ...
    private void TryInitServerLogic(ICoreServerAPI serverApi)
    {
        ...
        {
            serverApi.Logger.Warning("Crossbows: deferred server logic init for {0} ({1}) failed during load; will retry on demand. {2}",
                Code?.ToString() ?? "(unknown item)", GetType().Name, e.Message);
        }
    }
    ...
    private void TryInitClientLogic(ICoreClientAPI clientAPI)
    {
        ...
        {
            // Delay initialization until dependent systems are fully available.
        }
    }
}
